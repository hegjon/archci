...
"$top" --once --no-journal | grep "^signed: x86_64 63 pkg  aarch64 unreachable$" >/dev/null || fail "the signed line must show the released databases per arch: $("$top" --once --no-journal | grep ^signed)"
