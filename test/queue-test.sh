...
[[ $(gzip -dc "$xf" | sed '2s/,"exported":"[^"]*"//') == "$(grep -v '^id: ' <<<"$sse")" ]] || fail "the export is the live stream's bytes without the cursors, plus its own name in the job event: $(diff <(grep -v '^id: ' <<<"$sse") <(gzip -dc "$xf") | head -6)"
! gzip -dc "$xf" | grep -q '^id: ' || fail "an export carries no cursor lines"
