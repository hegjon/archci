...
"$master/archci-pkgs" | grep -q '^skipped 1-1 .* skip -$' || fail "archci-pkgs must list skip_build packages as skip"
...
ARCHCI_PKGBUILDS_URL=file://$pkgs2 "$master/archci-pkgs" lib32-thing | grep -q '^lib32-thing 1-1 [0-9a-f]* x86_64 multilib aur build -$' || fail "fork's package missing or wrong profile"
