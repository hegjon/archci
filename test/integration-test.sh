...
COLUMNS=200 ARCHCI_REMOTE_JOURNAL=$tmp/no-journal "$here/../master/archci-top" --once | grep -q " 3.7  1840M  2100M   5.0G  -         acl 1:2.3.2-1 | -" || fail "archci-top must show the job's cpu, memory and build size: $(ARCHCI_REMOTE_JOURNAL=$tmp/no-journal "$here/../master/archci-top" --once | grep worker-1)"
...
ARCHCI_REMOTE_JOURNAL=$tmp/no-journal "$here/../master/archci-top" --once | grep -q "^worker  *DigitalOcean  *x86_64  *0.10 .* 4  *1  *0$" || fail "archci-top must show an idle host with its last heartbeat and no active workers"
